...
def get_all_broadcasts(show_title):
    ...
    response = requests.get(
        STATION_INFO["player_search_url"] % urllib.parse.quote_plus(show_title),
        timeout=5,
    )
    ...
    all_broadcast_jsons = []
    for hit in sorted(
        results_json["hits"], key=lambda x: x["data"]["start"], reverse=True
    ):
...
def create_filename(broadcast_json):
    ...
    station_name = (
        STATION_INFO["name"]
        if not show_name.lower().startswith(STATION_INFO["name"].lower())
        else None
    )
    airdate_string = datetime.fromtimestamp(broadcast_json["start"] / 1000).strftime(
        "%Y-%m-%d %H:%M"
    )

    filename = (
        " ".join(filter(None, [station_name, show_name, airdate_string])) + ".mp3"
    )
...
def get_chapters(broadcast_json):
    ...
    for item_num, item in enumerate(
        sorted(broadcast_json["items"], key=lambda x: (x["start"], 1 / x["end"]))
    ):
        ...
        title = ": ".join(
            filter(None, [strip_html(item.get(i)) for i in ["interpreter", "title"]])
...
def get_keepmarks(broadcast_json):
    ...
    for mark in sorted(broadcast_json["marks"], key=lambda x: x["timestamp"]):
        ...
        if start is not None and end is not None:
            keepmarks.append(
                [start - broadcast_json["start"], end - broadcast_json["start"]]
            )
            start = None
            end = None
...
def remove_chapters_from_keepmark(keepmark, chapters):
    ...
    for chapter_num, chapter in enumerate(chapters):
        ...
        if chapter["start"] < keepmark[0]:
            return remove_chapters_from_keepmark(
                [chapter["end"], keepmark[1]], chapters[chapter_num:]
            )
        ...
        if chapter["end"] <= keepmark[1]:
            ...
            right = remove_chapters_from_keepmark(
                [min(chapter["end"], keepmark[1]), keepmark[1]], chapters[chapter_num:]
            )
            return [left] + right
...
def remove_chaptertypes_from_keepmarks(keepmarks, chapters, chapter_types):
    ...
    unwanted_chapters = [
        chapter for chapter in chapters if chapter["type"] in chapter_types
    ]
    ...
    with av.open(io.BytesIO(audio), format="mp3") as input_container:
        ...
        with av.open(output_buffer, "w", format="mp3") as output_container:
            ...
            output_stream = output_container.add_stream(
                "mp3", bit_rate=input_stream.bit_rate, rate=input_stream.rate
            )
...
def set_id3_tags(filepath, chapters, keepmarks, broadcast_json):
    ...
    tags.add(
        WORS(url=STATION_INFO["website"])
    )  # Official Internet radio station homepage
    tags.add(TCON(text=["Radio Recording"]))  # Content Description

    tags.add(
        TPE1(text=[strip_html(STATION_INFO["name"])])
    )  # Lead performer(s)/Soloist(s) -> "FM4"
    ...
    tags.add(
        TIT2(text=[broadcast_datetime.strftime("%Y-%m-%d %H:%M")])
    ...
    tags.add(
        TDAT(text=[broadcast_datetime.strftime("%d%m")])
    ...
    if image:
        tags.add(
            APIC(type=PictureType.COVER_FRONT, mime=image["mime"], data=image["data"])
        )
    ...
    tags.add(
        CTOC(
            ...
            child_element_ids=[
                chapter["id"] for chapter in chapters if not chapter["hidden"]
            ],
...
def main():
    ...
    parser.add_argument(
        "-c",
        "--cut",
        help='Cut all chapters of given types from recording, comma separated. Known types: B = Feature ("Beitrag"), J = Jingle, M = Music ("Musik"), N = News ("Nachrichten"), SO = Feature, W = Advertisement ("Werbung") (default: %(default)s)',
        default=None,
        metavar="TYPE",
    )
    parser.add_argument(
        "-i",
        "--ignore",
        help="Ignore recommended audio section removals. Typically News are removed/skipped this way (default: %(default)s)",
        action="store_true",
    )
    parser.add_argument(
        "-n",
        "--newest",
        help="Download newest broadcast only (default: %(default)s)",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "TargetDirectory",
        help="Directory to save the files in (default: %(default)s)",
        nargs="?",
        default=os.getcwd(),
    )
    ...
    CUT_CHAPTER_TYPES = (
        [x.strip().upper() for x in args.cut.split(",")] if args.cut else []
    )
    ...
    if not PYAV_AVAILABLE and (CUT_CHAPTER_TYPES or not IGNORE_KEEPMARKS):
        print(
            "PyAV not found, cutting audio not supported. Will continue to download complete broadcasts."
        )
        CUT_CHAPTER_TYPES = []
    ...
    for broadcast_json in all_broadcast_jsons:
        ...
        if CUT_CHAPTER_TYPES:
            keepmarks = remove_chaptertypes_from_keepmarks(
                keepmarks, chapters, CUT_CHAPTER_TYPES
            )
